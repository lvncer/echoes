...
##### 🚀 Phase 2: 音声処理基盤（次の実装対象）

**詳細**: [GitHub Issue #10 - Phase 2: 音声処理基盤の実装](https://github.com/lvncer/echoes/issues/10)

- 音声入力・録音機能
- 音声認識(STT): Web Speech API / OpenAI Whisper API
- 音声合成(TTS): Web Speech API / OpenAI TTS API
- 音声制御: プッシュトゥトーク、VAD（Voice Activity Detection）

##### 📋 Phase 3: リップシンク・アニメーション（予定）

- リップシンク: 音声同期口パクアニメーション
- 表情制御: 感情表現、ジェスチャー
- AI メッセージ連動: チャット応答に合わせたアニメーション
...
| 機能          | 採用技術           | 理由                       |
| ------------- | ------------------ | -------------------------- |
| 音声認識(STT) | Web Speech API     | ブラウザ標準、リアルタイム |
| 音声合成(TTS) | Web Speech API     | ブラウザ標準、簡単実装     |
| 音声制御      | プッシュトゥトーク | 手動制御、実装容易         |

**将来拡張（Phase 3 以降）**

| 機能          | 拡張技術            | 代替案                     | 理由             |
| ------------- | ------------------- | -------------------------- | ---------------- |
| 音声認識(STT) | OpenAI Whisper API  | Google Cloud Speech        | 精度・多言語対応 |
| 音声合成(TTS) | OpenAI TTS API      | VOICEVOX, Google Cloud TTS | 自然な音声品質   |
| リップシンク  | Three.js + 音素解析 | 既存ライブラリ             | カスタマイズ性   |
...
gantt
    ...
    section Phase 2: 音声処理（Web Speech API）
    音声入力基盤        :active, audio-base, 2025-06-25, 2025-07-05
    音声認識STT         :stt, 2025-07-01, 2025-07-10
    音声合成TTS         :tts, 2025-07-05, 2025-07-15
    音声処理統合        :audio-integration, 2025-07-10, 2025-07-20

    section Phase 3: アニメーション
    リップシンク        :lipsync, 2025-07-15, 2025-08-05
    表情・ジェスチャー  :animation, 2025-07-25, 2025-08-15

    section Phase 4: 音声処理拡張
    OpenAI Whisper/TTS  :openai-audio, 2025-08-01, 2025-08-20
    VOICEVOX統合        :voicevox, 2025-08-10, 2025-08-25

    section Phase 5: 高度機能
    設定画面UI          :settings, 2025-08-15, 2025-09-05
    データ管理          :data, 2025-08-25, 2025-09-10
```
...
2. **統合テスト**: 3D 表示・AI 連携・音声処理の統合動作テスト
3. **E2E テスト**: ユーザーシナリオベースの完全動作テスト
4. **パフォーマンステスト**: 負荷テスト・メモリリークテスト
...
1. **デバッグコード削除**: console.log 完全除去
2. **テスト環境構築**: Vitest + Testing Library 設定
3. **基本テストケース作成**: 3D モデル読み込み・AI 応答テスト
4. **エラーハンドリング強化**: try-catch 追加、ユーザーフィードバック改善

#### 中期実装項目

1. **音声録音機能**: MediaRecorder API 実装
2. **音声認識連携**: OpenAI Whisper API 統合
3. **音声合成機能**: OpenAI TTS API 統合
4. **リップシンク基盤**: 音素解析・口形状マッピング

この計画に従って、段階的に高品質な AI アバターチャットアプリケーションを完成させていきます。
